...
- **Always use issues.sh to close:** Run `bash .genesis/scripts/issues.sh close --id ISSUE_NUMBER` instead of raw `gh issue close`. The script automatically strips `in-progress` and `blocked` labels before closing, preventing stale labels from accumulating on closed issues.
- **Concurrency awareness:** Multiple event runs can fire simultaneously (e.g., closing an issue triggers both "issues: closed" and "issue_comment: created"). The events workflow serializes runs via `concurrency: genesis-events`, but a scheduled orchestrator run can still overlap with an event run. Always run `check-duplicate` before any `gh issue create` — it's your last safety net. If you discover duplicates, close the higher-numbered one immediately.
