...
- **No duplicate issues:** Before creating ANY issue (task, plan, or completion), run `gh issue list --repo OWNER/REPO --state all --search "TITLE_KEYWORDS" --limit 10` and check the output carefully. If a matching issue already exists (open OR closed), do NOT create a new one — reference the existing issue number instead. Milestone 2 produced 10 issues for 5 tasks because this check was skipped. Do not repeat this.
