...
- **Idle orchestrator runs burn LLM tokens** — The scheduled orchestrator runs every 6 hours even when a `needs:human` gate is open and there is nothing to do. With an average idle cost of ~$0.13/run, a 4-day human wait produces ~17 wasted runs (~$2). Fixed 2026-04-11 by adding a pre-flight `gh issue list` check in `genesis-orchestrator.yml`: if any `needs:human` issues are open, the Claude step is skipped entirely. The events workflow retains no such guard because a human closing an issue IS the trigger to advance.
- **`actions/create-github-app-token` input renamed** — In `create-github-app-token@v3`, the `app-id` input was deprecated in favour of `client-id`. The secret name (`GENESIS_APP_ID`) stays the same — only the workflow input key changes. All 4 workflows updated 2026-04-11. This eliminated a noisy deprecation warning that appeared in every single run.
